...
import com.floreantpos.ui.dialog.TimeSetupDialog;
...
public class OrderController implements OrderListener, CategorySelectionListener, GroupSelectionListener, ItemSelectionListener, ModifierSelectionListener {
	...
	public void payOrderSelected(Ticket ticket) {
		...
		if(ticket.getLeave()!=null){
			TimeSetupDialog timeSetupDialog = new TimeSetupDialog(Application.getPosWindow(), true);
			//timeSetupDialog.setTicket(ticket);
			timeSetupDialog.open();
			
			if (timeSetupDialog.isCanceled()) {
				return;
			}else{
				Date date=new Date( timeSetupDialog.getStime());
				ticket.setStartDate(date);
				Date endDate=new Date( timeSetupDialog.getEtime());
