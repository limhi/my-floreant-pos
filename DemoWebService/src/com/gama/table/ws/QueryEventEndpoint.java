...
public class QueryEventEndpoint {
  ...
  private static final Logger logger=Logger.getLogger(QueryEventEndpoint.class);
...
private JmsTemplate jmsTemplate;
  ...
  public QueryEventResponse handleHolidayRequest(@RequestPayload QueryEventRequest request)
      throws Exception {
	  logger.debug(request.getEventId());
	  logger.debug(request.getMNum());
	  logger.debug(request.getStartDate());
	  logger.debug(request.getEndDate());
